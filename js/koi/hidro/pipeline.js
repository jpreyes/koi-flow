...
import { ppDiseno, intensidad, grunsky, tablaIDF } from './idf.js?v=3';
import * as TC from './tc.js?v=3';
import * as Q from './caudales.js?v=3';
import { transponer } from './transposicion.js?v=3';
import { caudalesHU } from './hidrograma.js?v=3';
import { lineaNieveTemperatura, lineaNieveLatitud } from './linea_nieve.js?v=3';
...
export async function correrHidrologia(caso) {
  ...
  if (pp?.file) {                              // serie disponible → análisis de frecuencia (para mostrar)
    const j = await fetchJSON(pp.file + '?v=3', { contexto: `Serie pluvial ${pp.estacion || ''}`.trim() });
  ...
  if (caso.fluviometria) {
    ...
    const patron = await fetchJSON(fv.file + '?v=3', { contexto: `Serie fluviométrica ${fv.estacion || ''}`.trim() });
