...
import { fetchJSON, fetchJSONopcional, KoiDataError } from './fetch_json.js?v=5';
import { emit } from '../ui/bus.js?v=5';
...
export async function cargarCatalogo() {
  ...
  _catalogo = (await fetchJSONopcional('data/estaciones_dga.json?v=5',
...
export async function cargarSerie(est, tipo) {
  ...
  const j = await fetchJSONopcional(`data/series/dga/${archivo}?v=5`,
    { contexto: `Serie de ${nombre}` });
  if (j) return j;
  const back = await fetchJSONopcional(`data/series/dga/${bna}.json?v=5`,
    { contexto: `Serie de ${nombre}` });
  if (back) return back;
