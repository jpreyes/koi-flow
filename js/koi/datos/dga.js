...
import { fetchJSON, fetchJSONopcional, KoiDataError } from './fetch_json.js?v=4';
import { emit } from '../ui/bus.js?v=4';
...
export async function cargarCatalogo() {
  ...
  _catalogo = (await fetchJSONopcional('data/estaciones_dga.json?v=4',
...
export async function cargarSerie(est, tipo) {
  ...
  const j = await fetchJSONopcional(`data/series/dga/${archivo}?v=4`,
    { contexto: `Serie de ${nombre}` });
  if (j) return j;
  const back = await fetchJSONopcional(`data/series/dga/${bna}.json?v=4`,
    { contexto: `Serie de ${nombre}` });
  if (back) return back;
