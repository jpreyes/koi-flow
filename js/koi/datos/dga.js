...
import { fetchJSON, fetchJSONopcional, KoiDataError } from './fetch_json.js?v=6';
...
export async function cargarCatalogo() {
  ...
  _catalogo = (await fetchJSONopcional('data/estaciones_dga.json?v=6',
...
export async function cargarSerie(est, tipo) {
  ...
  const j = await fetchJSONopcional(`data/series/dga/${archivo}?v=6`,
    { contexto: `Serie de ${nombre}` });
  if (j) return j;
  const back = await fetchJSONopcional(`data/series/dga/${bna}.json?v=6`,
    { contexto: `Serie de ${nombre}` });
  if (back) return back;
